    ...
    public function run()
    {
        ...
        DB::table('critics')->insert(
            [
                ...
                ['comment'=> 'Que gran pelicula, muy recomendable','film_id' => $Harry->id,'user_id' => $Oscar->id],
                ['comment'=> 'Fantástica','film_id' => $exorcista->id,'user_id' => $agus->id],
                ['comment'=> 'Que miedo he pasado!','film_id' => $ouija->id,'user_id' => $agus->id],
                ['comment'=> 'Te deja con ganas de más','film_id' => $exorcista->id,'user_id' => $esteban->id],
                ['comment'=> 'Sin palabras me ha dejado','film_id' => $norespires->id,'user_id' => $esteban->id],
                ['comment'=> 'No la recomiendo....muy floja!','film_id' => $norespires->id,'user_id' => $esteban->id],
                ['comment'=> 'Una pasada','film_id' => $lobezno->id,'user_id' => $agus->id],
                ['comment'=> 'No puedes parar de reir en toda la pelicula','film_id' => $torrente->id,'user_id' => $jordi->id],
                ['comment'=> 'Este tipo de peliculas son lo peor','film_id' => $norespires->id,'user_id' => $esteban->id],
                ...
                ['comment'=> 'Solo en casa no estaba la verdad jajaja','film_id' => $norespires->id,'user_id' => $Julian->id],                                               
                ['comment'=> 'Pelicula bastante floja..','film_id' => $sully->id,'user_id' => $esteban->id],
                ['comment'=> 'Que pelicula más bonita','film_id' => $forrest->id,'user_id' => $agus->id],
                ['comment'=> 'Que gusto de película','film_id' => $forrest->id,'user_id' => $esteban->id],
                ['comment'=> 'No me acaba de convencer','film_id' => $sexto->id,'user_id' => $Oscar->id],
                ['comment'=> 'Estas películas son las mejores','film_id' => $besomuerte->id,'user_id' => $Julian->id],
                ...
                ['comment'=> 'Una obra maestra','film_id' => $pulp->id,'user_id' => $agus->id],
